...
$(document).ready(function () {
    $('#loginForm').on('submit', function (e) {
        ...
        // Correctly capture the input values
        var login = $('#username').val();
        var password = $('#password').val();

        // Use these variables in your AJAX request
        $.ajax({
            type: "POST",
            url: "login.php",
            data: { 
                loginUsername: login, // Use the 'login' variable for the username
                loginPassword: password // Use the 'password' variable for the password
            },
            success: function (response) {
                if (response.error) {
                    $('#loginError').text(response.errorMessage);
                } else {
                    // Now redirect or take action based on the response
                    if (response.firstLogin) {
                        // If it's the first login, store this info if needed and redirect
                        localStorage.setItem('firstLogin', true);
                        window.location.href = "homepage.php"; // Redirect to homepage
                    } else {
                        // Not first login, just redirect
...
$("#savePreferencesBtn").on("click", function() {
    // Assuming you have the user's ID or username to send in your request
    // This might be stored in session, passed in response, or embedded in the page
    var userId = document.body.getAttribute('data-user-id');

    $.ajax({
        type: "POST",
        url: "savePreferences.php", // Target PHP script to handle the data
        data: {
            userId: userId, // Assuming this is the user's ID
            dietaryPreferences: JSON.stringify(selectedDietaryPreference),
            ingredients: JSON.stringify(selectedIngredients),
            cuisinePreferences: JSON.stringify(selectedCuisine),
            cookingExperience: selectedExperience
        },
        success: function(response) {
